...
from ..app_layer import SyncApplication, ChartApplication
...
@keep_cli.command()
def create_events(ctx: typer.Context, dry_run: bool = False, resync: bool = False):
    """Ресинхронизация возможно только при создании событий, без режима dry run"""
    if dry_run:
        logger.info('Dry run.')
    ...
    ctx.obj.create_events(resync=resync)
...
def create_chart(ctx: typer.Context, type_,  w: int = 40, h: int = 10,
    ...
    ctx.obj.create_chart(groups, w, h, file)
